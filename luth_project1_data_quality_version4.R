...
# outdir <- "/Volumes/fsmresfiles/PrevMed/Projects/QDSC/Projects/07 Nigeria U54/Data quality reports/Project1_version4/Reports/LUTH/"
# 
# myurl <- "https://redcap.nubic.northwestern.edu/redcap/api/"
# mytoken  <-  read_csv(file.path(dir,"/Project 1/Analysis/Code/LUTH_P1_API.csv"))$Token #API token

#source(file.path(dir,'/Data quality reports/Project1_version4/R scripts/fields_v4.R'))

mymetadata = REDCapR::redcap_metadata_read(
  redcap_uri = myurl, token = mytoken
)$data

#====== updated to pull from data dictionary instead of source code "fields_v4.R"
LUTHp1_allFields <- mymetadata$field_name  #pull off variable names from data dictionary
LUTHp1_allFields2 <- LUTHp1_allFields[!LUTHp1_allFields %in% c("enr_hospital_id","enr_relphone", "enr_phone")]  #remove variables that have error
LUTHp1_finalFields <- c(LUTHp1_allFields2, 'enrollment_complete', 'hcc_diagnosis_complete', 'hiv_diagnosis_complete', 
...
enrform_fibro_y_missing <- sqldf("select record_id, redcap_event_name, enr_hiv_serology_y, enr_hcc_serology_y, enr_fibrosis_y,'Enrollment Form' as form
                               from luth_data where enr_fibrosis_y is null and redcap_event_name = ('enrollment_arm_1' or  'enrollment_arm_2')")
...
####HCC Diagnosis form
#############################have to edit this
hcc_y <- luth_data %>% dplyr::filter(luth_data$enr_hcc_serology_y == 1)
...
enrform_hcc_ae_missing <- luth_data %>% dplyr::filter(luth_data$enr_hcc_serology_y == 1 & (!hcc_artwashout_y %in% c(1,2,0) )) %>% select(record_id, redcap_event_name, hcc_artwashout_y )
...
enrform_hcc_bclc_missing <- luth_data %>% dplyr::filter(luth_data$enr_hcc_serology_y == 1 & !(luth_data$hcc_bclc %in% c(0,1,2,3,4 )))  %>% select(record_id, redcap_event_name, hcc_bclc )
...
a <- sqldf("SELECT record_id,redcap_event_name,form,group_concat(fields,'; ') as fields, sum(missing) as missing
...
#luth_data$hiv_cd4_initial <- as.character(as.numeric(luth_data$hiv_cd4_initial))

## HIV Diagnosis form - Enrollment arm 1 - Initial HIV RNA and CD4
luth_data_hivinitial_enr <-  luth_data %>% dplyr::filter( luth_data$redcap_event_name == "enrollment_arm_1" & luth_data$enr_hiv_serology == 1 &  (is.na(hiv_rna_pre_tx))) %>% 
  select(record_id,redcap_event_name, hiv_rna_pre_tx)#,hiv_cd4_initial) 
# | is.na(hiv_cd4_initial)))
luth_data_hivinitial_enr_miss <-  luth_data_hivinitial_enr %>% mutate(form ="HIV_diagnosis", missing = apply( luth_data_hivinitial_enr,1,sumna)) %>% select(record_id,redcap_event_name,form,missing)
###rename
names( luth_data_hivinitial_enr)[names( luth_data_hivinitial_enr) =="hiv_rna_pre_tx"]="Pre-treatment/baseline HIV RNA"
#names( luth_data_hivinitial_enr)[names( luth_data_hivinitial_enr) =="hiv_cd4_initial"]="Pre-ART CD4 T-Cell Count"

#####Function to list fields with 'NA' value
x =  luth_data_hivinitial_enr
fields <- test_proc(x)

luth_data_hivinitial_enr <- cbind( luth_data_hivinitial_enr_miss,fields)

## HIV Diagnosis form - Enrollment arm 1 - Prior Opportunistic infections
#luth_data_hiv_enr <- luth_data %>% dplyr::filter(luth_data$redcap_event_name == "enrollment_arm_1"  & luth_data$hiv_aids_y == 1 & (hiv_prioroi_y == 0|hiv_prioroi_y == 2))  %>%  mutate(aids_group="Yes") %>% select(record_id,redcap_event_name,enr_otherid,hiv_aids_y, hiv_prioroi_y,hiv_pjp_y,hiv_mac_y,hiv_cmv_y,hiv_pml_y,hiv_candida_y,hiv_crypto_y,hiv_nhl_y,hiv_ks_y, hiv_ade_other)
...
#luth_data_hiv2_enr <- luth_data %>% dplyr::filter(luth_data$redcap_event_name == "enrollment_arm_1"  & luth_data$hiv_aids_y == 0 & luth_data$hiv_prioroi_y == 1 & (hiv_pjp_y == 1 | hiv_mac_y == 1 | hiv_cmv_y == 1 | hiv_pml_y == 1 | hiv_candida_y == 1 | hiv_crypto_y == 1 | hiv_nhl_y == 1 | hiv_ks_y == 1 | hiv_ade_other == 1)) %>% select(record_id,redcap_event_name,enr_otherid,hiv_aids_y, hiv_prioroi_y,hiv_pjp_y,hiv_mac_y,hiv_cmv_y,hiv_pml_y,hiv_candida_y,hiv_crypto_y,hiv_nhl_y,hiv_ks_y, hiv_ade_other)
...
luth_data_hivinitial_enr2 <- luth_data %>% dplyr::filter(luth_data$redcap_event_name == "enrollment_arm_2" & luth_data$enr_hiv_serology == 1 & (is.na(hiv_rna_pre_tx))) %>% 
  select(record_id, redcap_event_name, hiv_rna_pre_tx) #| is.na(hiv_cd4_initial) #,hiv_cd4_initial
luth_data_hivinitial_enr2_miss <- luth_data_hivinitial_enr2 %>% mutate(form ="HIV_diagnosis", missing = apply(luth_data_hivinitial_enr2,1,sumna)) %>% select(record_id,redcap_event_name,form,missing)

###rename
names(luth_data_hivinitial_enr2)[names(luth_data_hivinitial_enr2) =="hiv_rna_pre_tx"]="Pre-treatment/baseline HIV RNA"
#names(luth_data_hivinitial_enr2)[names(luth_data_hivinitial_enr2) =="hiv_cd4_initial"]="Pre-ART CD4 T-Cell Count"

#####Function to list fields with 'NA' value
x = luth_data_hivinitial_enr2
fields <- test_proc(x)

luth_data_hivinitial_enr2 <- cbind(luth_data_hivinitial_enr2_miss,fields)
...
hiv_diagnosis_miss <- rbind(hiv_diagnosis_miss1,hiv_diagnosis_miss2,luth_data_hivinitial_enr,luth_data_hivinitial_enr2)
...
visit_ht <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_ex_height, redcap_event_name, form)
...
visit_wt <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_ex_weight, redcap_event_name, form)
...
###hiv and hcc missing
visit_hivy <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_hiv_y, redcap_event_name, form)
...
visit_hccy <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_hcc_y, redcap_event_name, form) 
...
###ecog missing
#hcc_diagnosis_y <- luth_data %>% dplyr::filter(enr_hcc_serology_y == "1" & (redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2")) %>% mutate(form = "Visit form") %>% select(record_id, form)
ecog_missing <- luth_data %>% dplyr::filter(!(luth_data$visit_hcc_ecog %in% c(0,1,2,3,4 )) &((redcap_event_name == "visit_arm_1") | (redcap_event_name == "visit_arm_2"))) %>% mutate(form = "Visit form")  %>% select(record_id,redcap_event_name,visit_hcc_ecog,form)
...
###fibroscan
fibroscan <-luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% select (record_id, redcap_event_name, visit_inv_fibro)
...
hiv_diagnosis_y <- luth_data %>% dplyr::filter(enr_hiv_serology_y == "1" & (redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2")) %>% mutate(hiv_diagnosis = "Yes") %>% select(record_id, enr_otherid, hiv_diagnosis)
hiv_treatment_y <- luth_data %>% dplyr::filter(visit_hiv_rx_y == "1")  %>% select (record_id,redcap_event_name,visit_hiv_rx_y, visit_hiv_rx_sd,visit_hiv_rx_reg)
...
hbv_treatment_y <- luth_data %>% dplyr::filter(visit_hbv_rx_y == "1")  %>% select (record_id,redcap_event_name,visit_hbv_rx_y, visit_hbv_rx_sd,visit_hbv_rx_reg)
hbv_record <- luth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2") %>% select(record_id, enr_otherid)
...
visit_j <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_sx_jaun_y, redcap_event_name, form) %>% dplyr::filter(is.na(visit_sx_jaun_y))
...
visit_enc <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_ex_liver_enc_y, redcap_event_name, form)
...
miss_visit_form <- ddply(miss_visit_form, .(record_id, redcap_event_name, form), summarise,fields = paste(fields, collapse="; "), missing = sum(missing))
...
hiv_diagnosis_n <- luth_data %>% dplyr::filter((enr_hiv_serology_y == "1") & (redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2")) %>% select(record_id, enr_otherid, enr_hiv_serology_y)
lab_result <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% select(record_id,redcap_event_name, visit_lab_cd4cnt_res, visit_lab_hivrna_res)
...
#visit_lab_tot_bil_res,visit_lab_tot_bil_res is null or
###rename
...
miss_lab_form <- ddply(miss_lab_form, .(record_id, redcap_event_name, form), summarise,fields = paste(fields, collapse="; "), missing = sum(missing))
...
luth_enr <- luth_data %>% dplyr::filter(redcap_event_name %in% c('enrollment_arm_1','enrollment_arm_2')) %>% select ('record_id', 'enr_otherid',
                                                                                                                     'enr_site','enr_enroll_d', 'enr_hiv_serology_y',
                                                                                                                     'enr_hcc_serology_y', 'redcap_event_name')

luth_oc <- luth_data %>% dplyr::filter(redcap_event_name %in% c('outcome_arm_1','study_end_arm_2')) %>% select(record_id, redcap_repeat_instrument, redcap_repeat_instance, fup_d, final_death_y,final_death_d,final_fu_d)
status <- luth_oc
...
luth_excel <- left_join(luth_enr, new_last_known, by = 'record_id')


outcome_var1 <- luth_excel %>% select(record_id, redcap_event_name, final_death_y, date_valid)
###rename
...
### MISSING DATA LISTING - Hard coding 'out of study' subjects
agg_missing <- sqldf("select i.record_id as Record_ID, u.enr_otherid as Other_ID, 
                     ...
                     from u54_id u
                     left join item_missing i on i.record_id =u.record_id
...
visit_bmi <- luth_data %>% dplyr::filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_sx_bmi, redcap_event_name, form)
...
#write_csv(visit_bmi, '~/Desktop/luth_invalid_values.csv')


##############################################
### Followup visit events and dates - added on 11/02/2021
...
rmarkdown::render('luth_dq_p1_version4.Rmd', output_dir = 'output/luth', run_pandoc = TRUE)
#rmarkdown::render('11022021_luth_dq_p1_KB.Rmd', output_dir = 'output/luth', run_pandoc = TRUE)

rmarkdown::render('luth_dq_invalid.Rmd', output_dir = 'output/luth', run_pandoc = TRUE)
###

new <- paste("output/luth/luth_dq_p1_version4_",Sys.Date() ,".html", sep = "")
new2 <- paste("output/luth/luth_dq_invalid_",Sys.Date() ,".html", sep = "")
#print(new)
file.rename('output/luth/luth_dq_p1_version4.html', new)
file.rename('output/luth/luth_dq_invalid.html', new2)
