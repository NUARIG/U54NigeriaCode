...
### This is saved as updated_juth_project2_dataquality_version3_KB_nk in fsmresfiles
rm(list = ls())
...
#--          Commented out markdown render at end of script
...
# outdir <- "/Volumes/fsmresfiles/PrevMed/Projects/QDSC/Projects/07 Nigeria U54/Data quality reports/Project2_version3/Reports/juth/"
# myurl <- "https://redcap.nubic.northwestern.edu/redcap/api/"
# mytoken  <-  read_csv(file.path(dir,"/Project 2/Analysis/Code/juth_P2_API.csv"))$Token #API token

#source(file.path(dir, "Data quality reports/Project2_version3/R scripts/fields_p2_juth.R"))
...
juth_data <- mydata
rec_oth_id <- juth_data %>% dplyr::filter((redcap_event_name == 'enrollment_arm_1')|(redcap_event_name == 'enrollment_arm_2')) %>% select(record_id, enr_otherid)
...
## Split events and merge to get one row per subject as apposed to one row per event
split_enr1 <- mydata %>% dplyr::filter(redcap_event_name == 'enrollment_arm_1') %>% dplyr::select(record_id,redcap_event_name,enr_site,enr_otherid,enr_enroll_d,enr_icc_diag,visit_hiv_y,hiv_year,icc_diag_d)
split_se1 <- mydata %>% dplyr::filter(redcap_event_name == 'study_end_arm_1') %>% dplyr::select(record_id,exclude_subject) %>% dplyr::filter(!is.na(exclude_subject))
split_enr2 <- mydata %>% dplyr::filter(redcap_event_name == 'enrollment_arm_2') %>% dplyr::select(record_id,redcap_event_name,redcap_repeat_instance,enr_site,enr_otherid,enr_enroll_d,enr_icc_diag,hiv_year)
split_vis2 <- mydata %>% dplyr::filter(redcap_event_name == 'visit_arm_2') %>% dplyr::select(record_id,redcap_repeat_instance,visit_hiv_y) %>% dplyr::filter(redcap_repeat_instance == 1)
split_icc2 <- mydata %>% dplyr::filter(redcap_event_name == 'icc_diagnosis_arm_2') %>% dplyr::select(record_id,redcap_repeat_instance,icc_diag_d) %>% dplyr::filter(redcap_repeat_instance == 1)
split_se2 <- mydata %>% dplyr::filter(redcap_event_name == 'study_end_arm_2') %>% dplyr::select(record_id,exclude_subject) %>% dplyr::filter(!is.na(exclude_subject))
split_list1 <- list(split_enr1, split_se1)
#split_list2 <- list(split_enr2, split_vis2, split_icc2,split_se1,split_se2)
enr1 <- Reduce(function(x, y) merge(x, y, all=TRUE), split_list1, accumulate=FALSE) 
#enr2 <- Reduce(function(x, y) merge(x, y, all=TRUE), split_list2, accumulate=FALSE) %>% mutate(redcap_event_name = 'enrollment_arm_2')
enr2 <- sqldf(" select a.record_id, a.redcap_event_name, a.enr_site, a.enr_otherid, a.enr_enroll_d,a.enr_icc_diag, b.visit_hiv_y,a.hiv_year, c.icc_diag_d, d.exclude_subject
                from split_enr2 a left join split_vis2 b on a.record_id = b.record_id
                                        left join split_icc2 c on a.record_id = c.record_id
                                        left join split_se2 d on a.record_id = d.record_id
                where d.exclude_subject is not null")
dat <- rbind(enr1,enr2)
dat <- distinct(dat) 
### Excluded records/subjects
ex_sub <- mydata %>% dplyr::filter(exclude_subject == 1) %>% select (record_id)
dat <- subset(dat, !(record_id %in% ex_sub$record_id)) %>% dplyr::filter(redcap_event_name %in% c('enrollment_arm_1','enrollment_arm_2'))

# with(dat,table(visit_hiv_y, enr_icc_diag,exclude=NULL))
dat3 <- dat %>%
  dplyr::mutate(hiv_y = base::ifelse((!is.na(hiv_year)) | visit_hiv_y == 1, "Yes", "No"),
                icc_y = base::ifelse((!is.na(icc_diag_d)) | enr_icc_diag == 1, "Yes", "No")) %>%
  dplyr::mutate(cohort = base::ifelse(redcap_event_name == "enrollment_arm_2" & hiv_y == "Yes", "HIV+/CIN",
                                      base::ifelse(hiv_y == "Yes" & icc_y == "Yes", "HIV+/ICC+",
                                                   base::ifelse(hiv_y == "Yes" & icc_y == "No", "HIV+/ICC-",
                                                                base::ifelse(hiv_y == "No" & icc_y == "Yes", "HIV-/ICC+",
                                                                             base::ifelse(hiv_y == "No" & icc_y == "No", "HIV-/ICC-", NA))))))

#Define cohorts for missing data report
cohorts <- dat3 %>% 
  select(record_id, redcap_event_name, enr_otherid, enr_icc_diag, hiv_year, visit_hiv_y, icc_diag_d, hiv_y, icc_y,cohort,exclude_subject) %>%
  arrange(record_id)

cohort_hivpos <- cohorts %>% dplyr::filter((cohort == 'HIV+/ICC+') | (cohort == 'HIV+/ICC-') | (cohort == 'HIV+/CIN')) %>% select(record_id, enr_otherid)
cohort_iccpos <- cohorts %>% dplyr::filter((cohort == 'HIV+/ICC+') | (cohort == 'HIV-/ICC+')) %>% select(record_id, enr_otherid)
cohort_cin <- cohorts %>% dplyr::filter((cohort == 'HIV+/CIN')) %>% select(record_id, enr_otherid)

## Data for cohort defining variables applying exclusion form
dat_wide <- subset(dat3, !(record_id %in% ex_sub$record_id))
##########updated 01/20/2022 - select variables in u54_id
### Add u54 subject ID and prep the data for R markdown
u54_id <- dat_wide %>% select(record_id,redcap_event_name, enr_otherid, enr_enroll_d, hiv_year, visit_hiv_y ,icc_diag_d,enr_icc_diag,cohort) #enr_enroll_d,
...
########Sexual history
enrform_sx_hx <- juth_data %>% dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name, enr_sexhx_age, enr_sexhx_num_partner, enr_sexhx_y)
...
enrform_cx_scrn <- juth_data %>%  dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name,enr_icc_scrhx_y,enr_icc_scrhx_test_y,enr_icc_scrhx_test_type,enr_icc_scrhx_test_age)
...
enrform_cx_scrn_pap <- juth_data %>%  dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name,enr_icc_scrhx_test_type, enr_icc_pap_hx_y, enr_icc_pap_hx_test_y, enr_icc_pap_hx_tmt) 
...
#####Risk factors
enr_risk_med <- juth_data %>% dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name,enr_icc_hpv_y, enr_icc_chly_y, enr_icc_tobacco_life,enr_icc_tobacco_prtnr,enr_icc_iccfhx_y)
...
enrform_hpv_conditions <- juth_data %>% dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id,redcap_event_name,enr_hpv_hpvimm, enr_oth_diab, enr_oth_htn, enr_oth_kid_dis)
...
miss_enr_form<- ddply(miss_enr_form, .(record_id, redcap_event_name, form), summarise,fields = paste(fields, collapse="<br/> "), missing = sum(missing))
...
if (nrow(juth_data_hiv_enr)>0) { juth_data_hiv_enr$hiv_prioroi_y = NA} else {juth_data_hiv_enr$hiv_prioroi_y = NULL}
juth_data_hiv2_enr <- sqldf(" select j.record_id,
                            ...
                            hiv_nhl_y = 1 or
                            hiv_ks_y = 1) ")
if (nrow(juth_data_hiv2_enr)>0) { juth_data_hiv2_enr$hiv_aids_y = NA} else {juth_data_hiv2_enr$hiv_aids_y = NULL}
...
miss_hiv_diagnosis<- ddply(miss_hiv_diagnosis, .(record_id, redcap_event_name, form), summarise,fields = paste(fields, collapse="; "), missing = sum(missing))
...
miss_icc_diagnosis <- ddply(miss_icc_diag, .(record_id, redcap_event_name, form), summarise,fields = paste(fields, collapse="; "), missing = sum(missing))
#############################################
#####Visit form
### Height missing
visit_ht <- juth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "visit_arm_2") %>% 
...
### Weight missing
visit_wt <- juth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "visit_arm_2") %>% 
...
###hiv and icc missing
visit_hivy <- juth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "visit_arm_2") %>% 
...
hiv_treatment_y <- juth_data %>% dplyr::filter(visit_hiv_rx_y == "1")  %>% 
...
###arm1
icc_diag_y_arm1 <- juth_data %>% dplyr::filter(redcap_event_name == 'enrollment_arm_1')  %>% 
...
icc_tmt_arm1 <-  juth_data %>% dplyr::filter(redcap_event_name == 'enrollment_arm_1' ) %>% 
  select(record_id,enr_otherid,visit_tmt_icc_type)
icc_tmt_arm1 <- icc_tmt_arm1 %>% dplyr::filter(icc_tmt_arm1$record_id %in% cohort_iccpos$record_id)
icc_tmt_arm2 <- juth_data %>% dplyr::filter(redcap_event_name == 'visit_arm_2') %>% 
...
icc_tmt_arm2 <- icc_tmt_arm2 %>% dplyr::filter(icc_tmt_arm2$record_id %in% cohort_iccpos$record_id)
...
#####cervical biopsy 
######have to check if we need to dplyr::filter this based on cohort identifying variables
###arm1
...
miss_visit_form <- ddply(miss_visit_form, .(record_id, redcap_event_name, form), summarise,fields = paste(fields, collapse="<br/>"), missing = sum(missing))
...
outcome <- juth_data %>% dplyr::filter((redcap_event_name == 'study_end_arm_1')|(redcap_event_name == 'study_end_arm_2')) %>% 
...
outcome_missing <- outcome_d %>% dplyr::filter(is.na(final_completed_d)) %>% select(record_id,redcap_event_name,final_completed_d,final_death_y, final_death_d,final_death_causes,final_withdraw_y, final_withdraw_d,final_withdraw_reason) 
...
outcome_trmt <- outcome_d %>% dplyr::filter ((!is.na(final_completed_d)) & (is.na(outcome_tmt))) %>% select(record_id, redcap_event_name, final_completed_d, outcome_tmt)
...
agg_missing <- sqldf(" select i.record_id as Record_id, 
                     ...
                     join u54_id u on i.record_id =u.record_id
...
# agg_missing <- ddply(agg_missing, .(Record_id, Other_ID), summarise, Form_fields = paste(Form_fields, collapse=" <br/> ", sep = "<br/>"), missing = sum(Missing))
...
# rmarkdown::render(input = "juth_dq_p2_KB.Rmd",
#                  output_dir = outdir, run_pandoc = TRUE)

############################################## 1114-1184--------------
### Followup visit events and dates - added on 01/20/2022
# u54_id <- u54_id %>% dplyr::mutate(hiv_y = base::ifelse((!is.na(hiv_year)) | visit_hiv_y == 1, "Yes", "No"),
#               icc_y = base::ifelse((!is.na(icc_diag_d)) | enr_icc_diag == 1, "Yes", "No"))
# u54_id <- u54_id %>% dplyr::mutate(cohort = base::ifelse(redcap_event_name == "enrollment_arm_2" & hiv_y == "Yes", "HIV+/CIN",
#                                     base::ifelse(hiv_y == "Yes" & icc_y == "Yes", "HIV+/ICC+",
#                                                  base::ifelse(hiv_y == "Yes" & icc_y == "No", "HIV+/ICC-",
#                                                               base::ifelse(hiv_y == "No" & icc_y == "Yes", "HIV-/ICC+",
#                                                                            base::ifelse(hiv_y == "No" & icc_y == "No", "HIV-/ICC-", NA))))))
...
u54_id$month_48 <- u54_id$enr_enroll_d %m+% months(48)
u54_id$month_60 <- u54_id$enr_enroll_d %m+% months(60)
...
###################################################################################
outcome_status <- mydata %>% filter((redcap_event_name %in% c('study_end_arm_1','study_end_arm_2'))& is.na(redcap_repeat_instance) ) %>% select(record_id,final_death_y)
# 1= Yes, 0= No, 2= Unknown
outcome_na <- outcome_status %>% filter(is.na(outcome_status$final_death_y))
...
u54_id <- u54_id[, c(1,2,3,15,9,4, 5, 6,7, 8,10, 11, 12, 13, 14)]
#select columns 
u54_id_sel <- u54_id %>% select( -hiv_year, -visit_hiv_y, -icc_diag_d, -enr_icc_diag)
rownames(u54_id_sel) <- NULL

source('dq_followupvisit_p2_0629.R')
...
# colnames(u54_id_sel) <- c("Record ID", "REDCap Event", "U54 SubjectID", "PEPID", "Has the patient Died?", "Enrollment Date","Cohort","Calculated 12 months visit","Calculated 24 months visit","Calculated 36 months visit","Calculated 48 months visit","Calculated 60 months visit")
# # "Cohort(enr_fibrosis_y)"
# 
rmarkdown::render('juth_dq_p2_updated_0629.Rmd', output_dir = 'output/juth', run_pandoc = TRUE)                  
new <- paste0("output/juth/juth_dq_p2_",Sys.Date() ,".html")
file.rename(paste0('output/juth/juth_dq_p2_updated_0629.html'), new)
