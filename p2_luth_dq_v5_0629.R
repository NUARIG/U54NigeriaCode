...
# outdir <- "/Volumes/fsmresfiles/PrevMed/Projects/QDSC/Projects/07 Nigeria U54/Data quality reports/Project2_version3/Reports/luth/"
# myurl <- "https://redcap.nubic.northwestern.edu/redcap/api/"
# mytoken  <-  read_csv(file.path(dir,"/Project 2/Analysis/Code/luth_P2_API.csv"))$Token #API token

#source(file.path(dir, "Data quality reports/Project2_version3/R scripts/fields_p2_luth.R"))
...
luth_data <- mydata
rec_oth_id <- luth_data %>% dplyr::filter((redcap_event_name == 'enrollment_arm_1')|(redcap_event_name == 'enrollment_arm_2')) %>% select(record_id, enr_otherid)
...
########Sexual history
enrform_sx_hx <- luth_data %>% dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name, enr_sexhx_age, enr_sexhx_num_partner, enr_sexhx_y)
...
enrform_cx_scrn <- luth_data %>%  dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name,enr_icc_scrhx_y,enr_icc_scrhx_test_y,enr_icc_scrhx_test_type,enr_icc_scrhx_test_age)
...
enrform_cx_scrn_pap <- luth_data %>%  dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name,enr_icc_scrhx_test_type, enr_icc_pap_hx_y, enr_icc_pap_hx_test_y, enr_icc_pap_hx_tmt) 
...
#####Risk factors
enr_risk_med <- luth_data %>% dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id, redcap_event_name,enr_icc_hpv_y, enr_icc_chly_y, enr_icc_tobacco_life,enr_icc_tobacco_prtnr,enr_icc_iccfhx_y)
...
enrform_hpv_conditions <- luth_data %>% dplyr::filter((redcap_event_name == "enrollment_arm_1")|(redcap_event_name == "enrollment_arm_2")) %>% select(record_id,redcap_event_name,enr_hpv_hpvimm, enr_oth_diab, enr_oth_htn, enr_oth_kid_dis)
...
if (nrow(luth_data_hiv_enr)>0) { luth_data_hiv_enr$hiv_prioroi_y = NA} else {luth_data_hiv_enr$hiv_prioroi_y = NULL}
luth_data_hiv2_enr <- sqldf(" select j.record_id,
                            ...
                            hiv_nhl_y = 1 or
                            hiv_ks_y = 1) ")
if (nrow(luth_data_hiv2_enr)>0) { luth_data_hiv2_enr$hiv_aids_y = NA} else {luth_data_hiv2_enr$hiv_aids_y = NULL}
...
#####Visit form
### Height missing
visit_ht <- luth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "visit_arm_2") %>% 
...
### Weight missing
visit_wt <- luth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "visit_arm_2") %>% 
...
###hiv and icc missing
visit_hivy <- luth_data %>% dplyr::filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "visit_arm_2") %>% 
...
hiv_treatment_y <- luth_data %>% dplyr::filter(visit_hiv_rx_y == "1")  %>% 
...
###arm1
icc_diag_y_arm1 <- luth_data %>% dplyr::filter(redcap_event_name == 'enrollment_arm_1')  %>% 
...
icc_tmt_arm1 <-  luth_data %>% dplyr::filter(redcap_event_name == 'enrollment_arm_1' ) %>% 
...
icc_tmt_arm2 <- luth_data %>% dplyr::filter(redcap_event_name == 'visit_arm_2') %>% 
...
outcome <- luth_data %>% dplyr::filter((redcap_event_name == 'study_end_arm_1')|(redcap_event_name == 'study_end_arm_2')) %>% 
...
# rmarkdown::render(input = "luth_dq_p2_KB.Rmd",
...
#source('dq_followupvisit_p2_0629_luth.R')
...
# 
rmarkdown::render('luth_dq_p2_updated_0629.Rmd', output_dir = 'output/luth', run_pandoc = TRUE)                  
new <- paste0("output/luth/luth_dq_p2_",Sys.Date() ,".html")
file.rename(paste0('output/luth/luth_dq_p2_updated_0629.html'), new)
